    ...
    for index, prob in enumerate(problem_list):
        ...
        if prob.get("img_base64"):
            try:
                ...
                temp_pic_path = f"temp_{index}.jpg"
...
final_user = st.text_input("✍️ 请输入实际检查人", value="") if selected_user == "✍️ 手动输入..." else selected_user
...
st.subheader(f"📷 第二步：录入问题 ({current_prob_idx}个)")
...
final_duty = st.text_input("✍️ 手动输入责任人", value="") if selected_duty == "✍️ 手动输入..." else selected_duty
...
if st.button("🚀 一键打包生成 PPT"):
    ...
    if out_file:
        with open(out_file, "rb") as file:
            st.download_button("📥 点击下载 PPT", data=file, file_name="巡场报告.pptx")