    ...
    for index, prob in enumerate(problem_list):
        ...
        if prob.get("img_base64"):
            try:
                ...
                with open(f"temp_prob_{index}.jpg", "wb") as f: f.write(img_bytes)
                current_slide.shapes.add_picture(f"temp_prob_{index}.jpg", Inches(0.52), Inches(2.3), width=Inches(2.95))
            except: pass
...
if st.button("➕ 确认并添加此条问题"):
    ...
    st.session_state.desc_buffer = "" # 添加成功后重置缓冲区
    st.success("🎉 问题已装箱！")
    st.rerun()
...
if st.button("🚀 一键打包生成 PPT"):
    out_file = build_multi_page_ppt(project_title, final_user, check_date.strftime("%Y/%m/%d"), st.session_state.problem_list)
    if out_file:
        with open(out_file, "rb") as file: 
            st.download_button("📥 点击下载 PPT", file, file_name=f"{project_title}-汇总报告.pptx")