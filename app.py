...
if st.button("➕ 确认并添加此条问题"):
    ...
    st.success("🎉 添加成功！请刷新查看列表。")
    st.rerun()

st.divider()

# 3. 汇总区 (即使列表为空，逻辑也能跑通，确保按钮在)
st.subheader("🚀 汇总与下载")
if len(st.session_state.problem_list) > 0:
    st.write(f"已暂存 {len(st.session_state.problem_list)} 个问题。")
        ...
        with st.spinner("生成中..."):
            ...
            if out_file:
                with open(out_file, "rb") as f:
                    st.download_button("📥 下载汇总报告 (若失败请在浏览器打开)", f, "最终报告.pptx")
else:
    st.info("⚠️ 请先添加问题，汇总按钮才会执行。")

if st.button("🗑️ 清空暂存箱"):
